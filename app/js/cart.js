...
export function showCart() {
  ...
  const totalMXN = cart.reduce((s, c) => s + (parseFloat(c.price) || 0) * c.qty, 0);
    ...
    <div class="cart-content">
      ...
      <div style="margin-top:15px;padding-top:15px;border-top:2px solid #eee;">
        <div style="display:flex;justify-content:space-between;font-size:18px;font-weight:800;">
          <span>Total</span><span>$${totalMXN.toFixed(0)} MXN</span>
        </div>
      ...
      <p style="text-align:center;font-size:11px;color:#aaa;margin-top:8px;">Tipo de cambio: 1 SOL ≈ $${SOL_RATE} MXN (Devnet)</p>
    </div>
  `;
...
export async function payWithSOL() {
  ...
  if (!window.solana || !window.solana.isPhantom) { alert('Instala Phantom Wallet.'); return; }
  ...
  try {
    ...
    await fetch(API+'/api/orders', {
      ...
      body: JSON.stringify({
        ...
        totalMXN, totalSOL, txSignature: signature
      })
    });
